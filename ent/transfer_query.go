...
// QueryFromAccount chains the current query on the "fromAccount" edge.
func (tq *TransferQuery) QueryFromAccount() *AccountQuery {
...
// QueryToAccount chains the current query on the "toAccount" edge.
func (tq *TransferQuery) QueryToAccount() *AccountQuery {
	query := &AccountQuery{config: tq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := tq.prepareQuery(ctx); err != nil {
			return nil, err
...
// Returns a *NotSingularError when exactly one Transfer entity is not found.
...
// Returns a *NotSingularError when exactly one Transfer ID is not found.
...
// the "fromAccount" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransferQuery) WithFromAccount(opts ...func(*AccountQuery)) *TransferQuery {
	query := &AccountQuery{config: tq.config}
...
// the "toAccount" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TransferQuery) WithToAccount(opts ...func(*AccountQuery)) *TransferQuery {
	query := &AccountQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
...
//		Scan(ctx, &v)
//
func (tq *TransferQuery) Select(field string, fields ...string) *TransferSelect {
...
func (tq *TransferQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	selector := builder.Select(t1.Columns(transfer.Columns...)...).From(t1)
	if tq.sql != nil {
		selector = tq.sql
		selector.Select(selector.Columns(transfer.Columns...)...)
	}
...
func (ts *TransferSelect) sqlQuery() sql.Querier {
	selector := ts.sql
